...
namespace BlogWebAPIApp.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public AuthService(BlogContext db, IPasswordService passwords, ITokenService tokens, IRepository<Guid, User> userRepository)
        {
            //_db = db;
            ...
            role)
        {
            ...
            if (await _userRepository.GetQueryable().AnyAsync(u => u.Email == email || u.Username == username))
        ...
        public async Task<(User user, string token)> Login(string emailOrUsername, string password)
        {
            ...
            var user = await _userRepository.GetQueryable().FirstOrDefaultAsync(u => u.Email == emailOrUsername || u.Username == emailOrUsername);
        ...
        public async Task<string> ForgotPassword(string email)
        {
            ...
            var user = await _userRepository.GetQueryable().FirstOrDefaultAsync(u => u.Email == email.Trim());
            if (user == null)
            {
        ...
        public async Task ResetPassword(string token, string newPassword)
        {
            ...
            var user = await _userRepository.GetQueryable().FirstOrDefaultAsync(u => u.Id == userId) ?? throw new InvalidOperationException("User not found");
