...
export class SearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (params['q'] || params['tag'] || params['category']) this.doSearch();
    });
  }

  doSearch(): void { this.page.set(1); this.fetch(1); }
  ...
  private fetch(p: number): void {
    ...
    this.postSvc.getPublished(p, this.pageSize, this.query.trim() || undefined, tag, cat).subscribe({
      next: r => {
        let items = r.items;
        if (this.selectedTags().length > 1)
          items = items.filter(post => this.selectedTags().every(s => post.tags.includes(s)));
        if (this.selectedCats().length > 1)
          items = items.filter(post => this.selectedCats().every(s => post.categories.includes(s)));
  ...
  removeTag(s: string): void { this.selectedTags.update(ts => ts.filter(t => t !== s)); this.doSearch(); }
  removeCat(s: string): void { this.selectedCats.update(cs => cs.filter(c => c !== s)); this.doSearch(); }

  goTo(p: number): void { this.page.set(p); this.fetch(p); window.scrollTo({ top: 0, behavior: 'smooth' }); }
